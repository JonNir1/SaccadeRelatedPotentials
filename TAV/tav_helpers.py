...
def match_boolean_events(
        is_gt: np.ndarray, is_pred: np.ndarray
) -> np.ndarray:
    """
    Matches between Ground-Truth and Predicted events, where events are represented as boolean arrays where `True`
    values indicate the sample where the event occurs. The matching is done such that there is minimal difference
    between the indices of the matched events.
    Returns a 2D array where each row is a matching pair of indices, where the first column is the GT index and the
    second column is the Predicted index.
